...
public class Sieve {
    ...
    public static void printsieve(int[] results, boolean[] prime) {
		...
		char1 = prime[j] ? 'T' : 'F';
		System.out.print(results[j] + "(" + char1 + ") ");
		j++;
	    }
	    System.out.println();
    }
    ...
    /**
     * @param size1 the size of the array to return.
     */
    
    public static boolean[] getTrueArray(int size1) {
    	
	boolean[] toReturn = new boolean[size1];
	...
	if (args.length > 0) {
	    ...
	    throw new IllegalArgumentException("User did not enter an arguement");
